...
<html  xmlns="http://www.w3.org/1999/xhtml">
		...
		function hiddenEditDiv() {
			...
			switch (roleCode) {         
			        case '5': 
			 			break;              
		             default:               
		                document.getElementById("btnDeleteInfo").style.display="none";  
		...
		*/
		function viewuploadphoto(action,code,msg){
			if(checkSelect(code,msg)){
				...
				uploadwindow.onbeforeunload = function(){
					if(check2()){commonSubmit('photouploadList.do?act=query');}
				}
			}
		}
		function refreshgamen() {
			setTimeout(function() {commonSubmit('photouploadList.do?act=query');},2000);
		}
		/*
		*	查看照片
		*/
		...
		function auditphoto(code,msg){
			if(checkSelect(code,msg)){
				...
				if(v){
					...
					document.forms[0].action = "photouploadEdit.do?act=auditphoto";
					disableAll(document);
				 	document.forms[0].submit();
			 		}
		 		});
				...
				if(v){
					...
					$.ajax({
							...
							success: function(data){
								text_info("核对备注添加成功！");
								commonSubmit('photouploadList.do?act=query');
							}
							});
							}
		 		});
	...
	<body onload="init();hiddenEditDiv();">
	<div style="height:440px;">
		<html:form action="photouploadList.do">
		...
		<bean:define id="statusList" name="PhotoUploadForm" property="statusList"></bean:define>
			<div id="content_all">
				...
				<div id="conten_query" >
					<table id="quert_table" cellpadding="1" cellspacing="1" border="0">
						...
						<!-- 	<td>
								...
								<td><input type="button" value="查询" class="commonButton" onclick="if(check2()){commonSubmit('photouploadList.do?act=query');}" /></td>
						</tr>
					</table>
				...
				<div name="button" class="content_button">
				 ...
				 <html:button styleId="btnDeleteInfo" property="btnDeleteInfo" value="删除信息" styleClass="commonButton" onclick="deleteinfo('UUIDS','请选择删除信息的条目')"/>
				 <html:button property="btnTaocan" value="核对备注" styleClass="commonButton" onclick="heduibeizhu('UUID','请单选择待核对备注的数据')"/>
				 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				 <html:button styleId="btnDeletePhoto" property="btnDeletePhoto" value="删除照片" style="color:white;background-color:red;height: 21px;" onclick="deletephoto('UUIDS','请选择删除照片的条目')"/>	
				</div>
			</div>
		</html:form>
		</div>
